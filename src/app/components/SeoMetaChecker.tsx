...
const tooltips = {
  title: (
    <div>
      ...
      <p>The title tag is one of the most important SEO elements. It should be:</p>
  ...
  ogTags: (
    <div>
      ...
      <p>Open Graph tags control how your content appears when shared on social media. Essential tags:</p>
      <ul className="list-disc list-inside mt-1 text-xs">
  ...
  twitterTags: (
    <div>
      ...
      <p>Twitter Card tags control how your content appears when shared on Twitter. Essential tags:</p>
      <ul className="list-disc list-inside mt-1 text-xs">
  ...
  canonical: (
    <div>
      ...
      <p>The canonical URL helps prevent duplicate content issues by telling search engines which URL is the main version of a page.</p>
      <p className="mt-2 text-xs">Example: <code>&lt;link rel=&quot;canonical&quot; href=&quot;https://example.com/page&quot; /&gt;</code></p>
    </div>
  )
};
...
export default function SeoMetaChecker() {
  ...
  const MetaSection = ({ title, tooltip, children }: { title: string; tooltip: React.ReactNode; children: React.ReactNode }) => (
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="flex items-center gap-2 mb-4">
        <h2 className="text-2xl font-bold text-gray-900">SEO Meta Checker</h2>
        <Tooltip content={
          <div>
            <p className="font-semibold mb-1">About SEO Meta Tags</p>
            <p>This tool analyzes your website&apos;s meta tags and provides recommendations for improving your SEO. It checks:</p>
            <ul className="list-disc list-inside mt-1 text-xs">
      ...
      {result && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <MetaSection title="Title Tag" tooltip={tooltips.title}>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={result.title.isOptimal ? "text-green-600" : "text-red-600"}>
                    {result.title.length} characters
                  </span>
                </div>
                <div className="text-sm font-mono bg-white p-2 rounded">
                  {result.title.content || 'No title tag found'}
                </div>
                {result.title.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside">
                    {result.title.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
            ...
            <MetaSection title="Meta Description" tooltip={tooltips.description}>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={result.description.isOptimal ? "text-green-600" : "text-red-600"}>
                    {result.description.length} characters
                  </span>
                </div>
                <div className="text-sm font-mono bg-white p-2 rounded">
                  {result.description.content || 'No meta description found'}
                </div>
                {result.description.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside">
                    {result.description.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
            ...
            <MetaSection title="H1 Tag" tooltip={tooltips.h1}>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={result.h1.isOptimal ? "text-green-600" : "text-red-600"}>
                    {result.h1.count} H1 tag{result.h1.count !== 1 ? 's' : ''} found
                  </span>
                </div>
                {result.h1.content && (
                  <div className="text-sm font-mono bg-white p-2 rounded">
                ...
                {result.h1.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside">
                    {result.h1.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
            ...
            <MetaSection title="Canonical URL" tooltip={tooltips.canonical}>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={result.canonical.isPresent ? "text-green-600" : "text-red-600"}>
                    {result.canonical.isPresent ? 'Canonical URL found' : 'No canonical URL found'}
                  </span>
                </div>
                ...
                {result.canonical.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside">
                    {result.canonical.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <MetaSection title="Open Graph Tags" tooltip={tooltips.ogTags}>
              <div className="space-y-2">
                ...
                {result.ogTags.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside mt-2">
                    {result.ogTags.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
            ...
            <MetaSection title="Twitter Card Tags" tooltip={tooltips.twitterTags}>
              <div className="space-y-2">
                ...
                {result.twitterTags.recommendations.length > 0 && (
                  <ul className="text-sm text-red-600 list-disc list-inside mt-2">
                    {result.twitterTags.recommendations.map((rec, i) => (
                      <li key={i}>{rec}</li>
                    ))}
                  </ul>
                )}
